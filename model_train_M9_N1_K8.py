# 系统模型：一个边长为 D=1KM 的正方形区域内,分布着个数为 M=9 的AP, 天线数 N=1
# 以及随机均匀分布着个数为 K=4 的单天线用户的 Cell-free 大规模MIMO系统
import torch
...
Path = 'Model_param_MMSE_M9_N1_K8_simulation.pkl'
...
for t in range(param["times"]):
    ...
    global_loss = Train_loss/len(User_set)
    global_acc = Train_acc / len(User_set)
    training_loss_ave.append(global_loss)
    training_acc_ave.append(global_acc)
    ...
    print(f"Number of iterations: {t+1},  global loss: {global_loss:.6f},  global acc: {100*global_acc:.6f}%")
...
# 要复制的目标文件目录
# 要复制的目标文件目录
cp_excel_file_path = ".\M9_N1_K8_Loss_Acc.xlsx"
